...
上述例子中，当 `ptr1` 离开作用域，`Resource` 不会被释放，因为 `Resource` 的 `m_ptr` 共享 `Resource`。这个时候 `Resource` 能被释放的唯一办法就是设置 `m_ptr` 指向其它（这样就没有任何贡献 `Resource` 的了）。但是不能访问 `m_ptr` 因为 `ptr` 离开了最重要，所以不再可能做到了。`Resouce` 变为了内存泄漏。
