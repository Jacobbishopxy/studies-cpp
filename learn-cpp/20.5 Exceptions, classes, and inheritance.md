...
构造函数是另一个使用异常非常有用的地方。如果一个构造函数因为某些原因（例如用户无效输入）必须失败时，简单的抛出一个异常就可以指示创建对象失败了。这种情况下对象的构造流产了，所有的类成员（已被创建的，以及之前在构造函数体内初始化的）会被照常被解构。

然而类的解构函数永远不会被调用（因为对象从未完成构造函数）。因为解构函数从未执行，因此不可依赖解构函数来清理已被分配的内存。
...
幸运的是，还有一个更好的办法。利用了类成员即使在构建函数失败时也会被解构的优势，如果资源分配处于类成员之中（而不是处于构造函数之中），那么这些成员可以在被解构时清理自身。
...
上述代码中，当类 A 抛出一个异常，所有 A 的成员都被解构，`m_member` 的解构函数被调用，提供了清理其分配的任何资源的机会。
...
前一种情况中，如果 Foo 构造函数在 ptr 分配动态内存后失败了，Foo 则需要负责清理，这会带来挑战。后一种情况中，Foo 构造函数在 ptr 分配动态内存后失败了，ptr 的解构函数会执行并将内存返回给系统。Foo 不需要做任何的显示清理，因为资源处理被委派给了符合 RAII 的成员！
